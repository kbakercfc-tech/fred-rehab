         ...
         db.all("PRAGMA table_info(exercises)", (err, columns) => {
            ...
            console.log("Migration is required. Starting the process...");
             ...
             db.exec(migrationScript, function(err) {
                 if (err) {
                     console.error("An error occurred during t migration:", err.message);
                     console.log("Attempting to roll back changes...");
                     db.exec('ROLLBACK;');
                 } else {
                     console.log("\nMigration successful!");
                     console.log("The 'exercises' table has be updated.");
                     console.log("The 'repetitions' column has been removed and 'weights_done' is set to 'No' for all previo entries.");
                 }
                 db.close((err) => {
