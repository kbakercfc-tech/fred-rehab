...
// This should point to the database you want to migrate.
...
db.serialize(() => {
    ...
    db.get("SELECT name FROM sqlite_master WHERE type='table' AND name='exercises'", (err, table) => {
        ...
        // Table exists, now check for the 'repetitions' column
        db.all("PRAGMA table_info(exercises)", (err, columns) => {
            ...
            const needsMigration = columns.some(col => col.name === 'repetitions');
